...
A blocking call is handled differently. When other coroutines are live, a blocking call is offloaded to a background worker pool: the calling coroutine cooperatively parks until the result is ready, and its siblings keep running meanwhile — the blocking call no longer freezes the actor. With nothing else to schedule the call simply runs inline on the actor thread, so a program that uses no `go` is unaffected. The offloaded calls are `Os.run` and `Os.cwd`; the waiting `IO` calls (`read_file`, `write_file`, `append_file`, the byte variants, `list_dir`, `mkdir`, `remove`, `read_line`); the waiting `Net` calls (`accept`, `connect`, `connect_tls`, `send_to`, `recv_from`, `read`, `write`, `read_exact`, `read_line`, `read_until`, `read_all`); and the cross-actor waits `Channel.send`, `Channel.recv`, `select`, and `join` on a `Task`. Fast non-waiting calls (`IO.exists`/`is_dir`/`is_file`/`stat`, `Net.listen`/`bind`/`local_addr`/`peer_addr`/`set_timeout`/`close`, `Channel.try_recv`/`close`) stay inline. One consequence: a green thread may now `Channel.recv` from a sibling green thread in the same actor without deadlocking — the receive parks cooperatively rather than sleeping the shared OS thread.
