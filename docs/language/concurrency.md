...
A blocking call is handled differently. When other coroutines are live, a blocking call is offloaded to a background worker pool: the calling coroutine cooperatively parks until the result is ready, and its siblings keep running meanwhile — the blocking call no longer freezes the actor. With nothing else to schedule the call simply runs inline on the actor thread, so a program that uses no `go` is unaffected. `Os.run`, the waiting `IO` calls (`read_file`, `write_file`, `append_file`, the byte variants, `list_dir`, `mkdir`, `remove`, `read_line`), `Os.cwd`, and the waiting `Net` calls (`accept`, `connect`, `connect_tls`, `send_to`, `recv_from`, `read`, `write`, `read_exact`, `read_line`, `read_until`, `read_all`) are all offloaded; fast non-waiting calls (`IO.exists`/`is_dir`/`is_file`/`stat`, `Net.listen`/`bind`/`local_addr`/`peer_addr`/`set_timeout`/`close`) stay inline. (A cross-actor channel `recv` still stalls every coroutine, and will be converted next.)
