...
The waiting calls — `accept`, `connect`, `connect_tls`, `send_to`, `recv_from`, `read`, `write`, `read_exact`, `read_line`, `read_until`, `read_all` — are *blocking* natives: inside a green thread they are offloaded to a worker pool, so a coroutine waiting on the network does not stall the actor's siblings (see [concurrency](../language/concurrency.md)). The non-waiting calls (`listen`, `bind`, `local_addr`, `peer_addr`, `set_timeout`, `close`) run inline.
