...
The `JSON` module reads and writes JSON text. It exposes no value type of its own: `parse` returns ordinary tigr values (`null`, `Bool`, `Float`, `String`, `Array`, `Object`) and `stringify` accepts the same. It is ambient, so a bare module name works without an `import`. Numbers are always parsed as `Float`, the convention JSON itself follows, so an integer-valued number comes back as `Float`.
